...
var _ = Describe("CheckResource", func() {
	Context("when ATC request succeeds", func() {
		...
		BeforeEach(func() {
			...
			expectedURL := "/api/v2/teams/some-team/pipelines/mypipeline/resources/myresource/check"
		...
		It("sends check resource request to ATC", func() {
			check, found, err := team.CheckResourceV2("mypipeline", "myresource", atc.Version{"ref": "fake-ref"})
	...
	Context("when pipeline or resource does not exist", func() {
		...
		It("returns a ResourceNotFoundError", func() {
			_, found, err := team.CheckResourceV2("mypipeline", "myresource", atc.Version{"ref": "fake-ref"})
			Expect(err).NotTo(HaveOccurred())
	...
	Context("when ATC responds with an error", func() {
		...
		It("returns an error", func() {
			_, _, err := team.CheckResourceV2("mypipeline", "myresource", atc.Version{"ref": "fake-ref"})
