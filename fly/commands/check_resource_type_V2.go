...
type CheckResourceTypeCommandV2 struct {
...
func (command *CheckResourceTypeCommandV2) Execute(args []string) error {
	...
	check, found, err := target.Team().CheckResourceTypeV2(command.ResourceType.PipelineName, command.ResourceType.ResourceName, version)
	if err != nil {
		return err
	}
