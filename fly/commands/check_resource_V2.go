...
type CheckResourceCommandV2 struct {
...
func (command *CheckResourceCommandV2) Execute(args []string) error {
	...
	check, found, err := target.Team().CheckResourceV2(command.Resource.PipelineName, command.Resource.ResourceName, version)
	if err != nil {
		return err
	}
