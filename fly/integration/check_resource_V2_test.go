...
var _ = Describe("CheckResourceV2", func() {
	var (
		flyCmd          *exec.Cmd
		check           atc.Check
	...
	Context("when ATC request succeeds", func() {
		...
		BeforeEach(func() {
			expectedURL := "/api/v2/teams/main/pipelines/mypipeline/resources/myresource/check"
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-experimental", "-r", "mypipeline/myresource", "-f", "ref:fake-ref")
	...
	Context("when version is omitted", func() {
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-experimental", "-r", "mypipeline/myresource")
				sess, err := gexec.Start(flyCmd, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
	...
	Context("when watching the check succeed", func() {
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-experimental", "-r", "mypipeline/myresource", "-w")
				sess, err := gexec.Start(flyCmd, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
	...
	Context("when specifying multiple versions", func() {
		...
		It("sends correct check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-experimental", "-r", "mypipeline/myresource", "-f", "ref1:fake-ref-1", "-f", "ref2:fake-ref-2")
				sess, err := gexec.Start(flyCmd, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
