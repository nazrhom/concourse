...
var _ = Describe("CheckResourceTypeV2", func() {
	var (
		flyCmd          *exec.Cmd
		check           atc.Check
	...
	Context("when version is specified", func() {
		BeforeEach(func() {
			expectedURL := "/api/v2/teams/main/pipelines/mypipeline/resource-types/myresource/check"
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-type-experimental", "-r", "mypipeline/myresource", "-f", "ref:fake-ref")
	...
	Context("when version is omitted", func() {
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-type-experimental", "-r", "mypipeline/myresource")
				sess, err := gexec.Start(flyCmd, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
	...
	Context("when watching the check succeed", func() {
		...
		It("sends check resource request to ATC", func() {
			Expect(func() {
				flyCmd = exec.Command(flyPath, "-t", targetName, "check-resource-type-experimental", "-r", "mypipeline/myresource", "-w")
				sess, err := gexec.Start(flyCmd, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
